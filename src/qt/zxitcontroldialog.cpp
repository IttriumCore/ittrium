...
#include "ui_zxitcontroldialog.h"
...
std::list<std::string> ZXitControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZXitControlDialog::listMints;

ZXitControlDialog::ZXitControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZXitControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZXitControlDialog::~ZXitControlDialog()
{
    delete ui;
}

void ZXitControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZXitControlDialog::updateList()
{
...
void ZXitControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZXitControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZXit_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZXitControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZXitControlDialog::GetSelectedMints()
{
...
void ZXitControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
