...
void OverviewPage::updateObfuscationProgress()
{
    ...
    QString strAnonymizeIttriumAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeIttriumAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if (currentBalance == 0) {
        ...
        strAnonymizeIttriumAmount = strAnonymizeIttriumAmount.remove(strAnonymizeIttriumAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeIttriumAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    ...
    if (nMaxToAnonymize > nAnonymizeIttriumAmount * COIN) nMaxToAnonymize = nAnonymizeIttriumAmount * COIN;

    if (nMaxToAnonymize == 0) return;

    if (nMaxToAnonymize >= nAnonymizeIttriumAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                              .arg(strAnonymizeIttriumAmount));
        strAnonymizeIttriumAmount = strAnonymizeIttriumAmount.remove(strAnonymizeIttriumAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeIttriumAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    } else {
