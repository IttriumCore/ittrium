                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zXIT ought to be enough for anybody." - Bill Gates, 2017
    ui->zXITpayAmount->setValidator(new QDoubleValidator(0.0, 21000000.0, 20, this));
    ...
    ui->labelzXITSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ...
        ui->pushButtonSpendzXIT->setEnabled(false);
        ui->pushButtonSpendzXIT->setToolTip(tr("zXIT is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zXITpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzXIT_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zXIT is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zXIT...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zXIT in ") +
...
void PrivacyDialog::on_pushButtonSpendzXIT_clicked()
{
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zXIT
        sendzXIT();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zXIT
    sendzXIT();
}

void PrivacyDialog::on_pushButtonZXITControl_clicked()
{
    ZXitControlDialog* zXitControl = new ZXitControlDialog(this);
    zXitControl->setModel(walletModel);
    zXitControl->exec();
}

void PrivacyDialog::setZXitControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzXITSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzXIT()
{
    QSettings settings;

    // Double is allowed now
    double dAmount = ui->zXITpayAmount->text().toDouble();
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid Ittrium Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    // Warn for additional fees if amount is not an integer and change as zXIT is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " XIT </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zXIT</b>";
    ...
    // use mints from zXIT selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZXitControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZXitControlDialog::GetSelectedMints();
    }
    ...
    // Clear zxit selector in case it was used
    ZXitControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zXIT Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " XIT, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zXIT Mint");
    ...
    // Clear amount to avoid double spending when accidentally clicking twice
    ui->zXITpayAmount->setText("0");
    ui->labelzXITSelected_int->setText("0");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zXIT </b>";
    ...
    CAmount xitBalance = balance - (immatureBalance + nLockedBalance);
    if (xitBalance < 0) xitBalance = 0;

    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zXIT "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zXIT "));
    ui->labelzXITAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, xitBalance, false, BitcoinUnits::separatorAlways));
    ...
    ui->labelZsupplyAmount->setText(QString::number(chainActive.Tip()->GetZerocoinSupply()/COIN) + QString(" <b>zXIT </b> "));
    for (auto denom : libzerocoin::zerocoinDenomList) {
        ...
        QString strSupply = QString::number(nSupply) + " x " + QString::number(denom) + " = <b>" +
                            QString::number(nSupply*denom) + " zXIT </b> ";
        switch (denom) {
            case libzerocoin::CoinDenomination::ZQ_ONE:
...
{
    ui->labelzXITSyncStatus->setVisible(fShow);
}
