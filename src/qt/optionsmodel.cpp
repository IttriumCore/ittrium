...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::XIT);
    ...
    if (!settings.contains("nObfuscationRounds"))
        settings.setValue("nObfuscationRounds", 3);

    if (!settings.contains("nAnonymizeIttriumAmount"))
        settings.setValue("nAnonymizeIttriumAmount", 1000);

    nAnonymizeIttriumAmount = settings.value("nAnonymizeIttriumAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeIttriumAmount"))
        SoftSetArg("-anonymizeittriumamount", settings.value("nAnonymizeIttriumAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in ittrium.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeIttriumAmount:
            return QVariant(nAnonymizeIttriumAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeIttriumAmount:
            ...
            settings.setValue("nAnonymizeIttriumAmount", nAnonymizeIttriumAmount);
            emit anonymizeIttriumAmountChanged(nAnonymizeIttriumAmount);
            break;
        case CoinControlFeatures:
